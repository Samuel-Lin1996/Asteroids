...
public class Polygon {
	...
	public boolean collision(Polygon a){
		for(int i=0; i<a.shape.length-1; i++){
			Point temp = new Point(a.position.getX() -((a.shape[i].getX() + a.shape[i+1].getX())/2), a.position.getY() - ((a.shape[i].getY() + a.shape[i+1].getY())/2));
			if(this.contains(temp)){
				return true;
			}
		}
		for(int j=0; j<this.shape.length-1; j++){
			Point shiptemp = new Point(this.position.getX() - ((this.shape[j].getX() + this.shape[j+1].getX())/2), this.position.getY() - ((this.shape[j].getY() + this.shape[j+1].getY())/2));
			if(a.contains(shiptemp)){
				return true;
			}
		}
		return false;
  ...
  methods in this class that are not private. They can't be used anywhere else.
	...
	public void paint(Graphics brush){
		...
		for(int i= 0; i<a.length; i++){
			xpoints[i] = (int)a[i].getX();
			ypoints[i] = (int)a[i].getY();
		}
		brush.drawPolygon(xpoints,ypoints,a.length);
	}
}
